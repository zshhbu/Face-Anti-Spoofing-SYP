...
# class CommonBlock(nn.Module):
#     def __init__(self, in_channel, out_channel, stride, basic_conv=Conv2d_cd):  # 普通Block简单完成两次卷积操作
#         super(CommonBlock, self).__init__()
...
#
#         x += identity  # 两路相加
#         return F.relu(x, inplace=True)  # 添加激活函数输出, inplace=True
#
#
#
# class SpecialBlock(nn.Module):  # 特殊Block完成两次卷积操作，以及一次升维下采样
#     def __init__(self, in_channel, out_channel, stride, basic_conv=Conv2d_cd):  # 注意这里的stride传入一个数组，shortcut和残差部分stride不同
#         super(SpecialBlock, self).__init__()
#         self.change_channel = nn.Sequential(  # 负责升维下采样的卷积网络change_channel
...
#
#         x += identity
#         return F.relu(x, inplace=True)  # 输出卷积单元  , inplace=True
...
class ResNet18(nn.Module):
    def __init__(self, classes_num=2):
        ...
        self.pool = nn.AdaptiveMaxPool2d(output_size=(4, 4))
    ...
    def forward(self, x_rgb):
        x_rgb = self.prepare(x_rgb)  # 16  # 三通道 64
        x = self.layer1(x_rgb)  # 16
        x = self.cbam2(x) * x + x
