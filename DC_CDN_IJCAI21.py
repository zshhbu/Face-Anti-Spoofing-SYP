...
# 假设你已经创建了模型实例 my_model
my_model = C_CDN().to(device="cuda")
checkpoint = torch.load(r"D:\BaiduNetdiskDownload\CDCN-master\CDCN-master\checkpoints\ra_2.pth")

# 将模型的权重加载到模型中
my_model.load_state_dict(checkpoint)
# 载入图像并进行预处理（确保图像大小和通道数与模型兼容）
input_image = Image.open(r"D:\BaiduNetdiskDownload\replayattack\test\attack_cut\client009_session01_webcam_authenticate_adverse_1\frame_0001.png")  # 替换为你的输入图像文件名
...
input_tensor = preprocess(input_image).unsqueeze(0)  # 增加一个批次维度
input_tensor = input_tensor.to(device='cuda')  # 将输入张量移动到与模型相同的设备
# 使用模型进行推断
with torch.no_grad():
    depth_map = my_model(input_tensor)

# depth_map 是一个张量，可以包含深度图像的预测结果

# 将深度图像的张量转换为可视图像
depth_map = depth_map.squeeze().cpu().numpy()  # 去掉批次维度，并将张量移到 CPU
# 现在 depth_map 是一个 NumPy 数组

# 保存深度图像为图像文件
depth_image = Image.fromarray((depth_map * 255).astype('uint8'))  # 假设深度在 [0, 1] 范围内
depth_image.save('output_depth_image.png')  # 保存为图像文件

# 如果你想在程序中进一步使用深度图像，请使用 depth_map 变量
